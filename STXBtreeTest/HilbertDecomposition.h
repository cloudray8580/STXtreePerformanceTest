...
void inline sub_range_decompose(unsigned max_bits, unsigned current_bits, int down_x, int up_x, int down_y, int up_y, int center_x, int center_y, direction pre, direction dir, quadrant quad_, vector<interval> &intervals) { // here we should use stack instead
	...
	int half_region = 1 << (current_bits - 1);
		...
		case clockwise:
			...
			case up_:
				...
				if (up_x >= center_x && up_y >= center_y) { // upper right
					...
					sub_range_decompose(max_bits, current_bits, max(center_x, down_x), up_x, max(center_y, down_y), up_y, center_x + half_region, center_y + half_region, parent_dir, child_dir, quad, intervals);
				}
				if (up_x >= center_x && down_y < center_y) { // lower right
					quad = lowerright;
					child_dir = next_level_direction[parent_dir][quad];
					sub_range_decompose(max_bits, current_bits, max(center_x, down_x), up_x, down_y, min(center_y - 1, up_y), center_x + half_region, center_y - half_region, parent_dir, child_dir, quad, intervals);
				}
				if (down_x < center_x && down_y < center_y) { // lower left
					quad = lowerleft;
					child_dir = next_level_direction[parent_dir][quad];
					sub_range_decompose(max_bits, current_bits, down_x, min(center_x - 1, up_x), down_y, min(center_y - 1, up_y), center_x - half_region, center_y - half_region, parent_dir, child_dir, quad, intervals);
				}
				if (down_x < center_x && up_y >= center_y) { // upper left
					quad = upperleft;
					child_dir = next_level_direction[parent_dir][quad];
					sub_range_decompose(max_bits, current_bits, down_x, min(center_x - 1, up_x), max(center_y, down_y), up_y, center_x - half_region, center_y + half_region, parent_dir, child_dir, quad, intervals);
				}
				break;
			case left_:
				parent_dir = left_;
...
vector<interval> inline mergeContinousIntervals(vector<interval> &intervals) {
...
vector<interval> inline GetIntervals(unsigned max_bits, unsigned lower_x, unsigned upper_x, unsigned lower_y, unsigned upper_y) {
	vector<interval> intervals;
	sub_range_decompose(max_bits, max_bits, lower_x, upper_x, lower_y, upper_y, 1 << (max_bits - 1), 1 << (max_bits - 1), none_dir, left_, none_quad, intervals);
	intervals = mergeContinousIntervals(intervals);
	return intervals;
}

int inline getTotalIntervalsCount(unsigned max_bits, unsigned lower_x, unsigned upper_x, unsigned lower_y, unsigned upper_y) {
	vector<interval> intervals;
	sub_range_decompose(max_bits, max_bits, lower_x, upper_x, lower_y, upper_y, 1 << (max_bits - 1), 1 << (max_bits - 1), none_dir, left_, none_quad, intervals);
	intervals = mergeContinousIntervals(intervals);
